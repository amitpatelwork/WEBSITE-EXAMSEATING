...
<body>
  ...
  $sql="select * from studentdetailadmin";
  $result = $con->query($sql);
  if ($result->num_rows > 0) {
    // output data of each row
    $i=1;
   echo "<table border='4' width='900px'><tr><th>Sr. No.</th><th>Name</th><th>COURSE_BRANCH</th><th>USER_NAME</th><th>PASSWORD</th></tr>";
    while($row = $result->fetch_assoc()) {
  echo "<tr><td>".$i."</td><td>".$row['NAME']."</td><td>".$row['COURSE_BRANCH']."</td><td>".$row['USER_NAME']."</td><td>".$row['PASSWORD']."</td><tR>";
  $i++;
    }
  }
  ?>
