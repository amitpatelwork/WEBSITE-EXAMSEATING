...
<body>
    ...
    <?php
  echo "<h2>TEACHER RECORD </h2>";
  $con=new mysqli("localhost","root","","pbl2project");
  $sql="select * from teacherdetailadmin";
  $result = $con->query($sql);
  if ($result->num_rows > 0) {
    $i=1;
  echo "<table border='5' width='1200px'><tr><th>ID.</th><th>Name</th><th>TEACHER ID</th><th>DEPARTMENT</th></tr>";
    while($row = $result->fetch_assoc()) {
  echo "<tr><td>".$i."</td><td>".$row['NAME']."</td><td>".$row['TEACHER_ID']."</td><td>".$row['DEPARTMENT_1']."</td><tr>";
  $i++;
    }
  }
  ?>
